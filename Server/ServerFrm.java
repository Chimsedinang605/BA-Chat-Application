...
public class ServerFrm extends javax.swing.JFrame {
    ...
    Connection getConnection() {
        try {
            return DriverManager.getConnection("jdbc:mysql://localhost:3306/messenger_sk", "root", "123456");
        } catch (Exception e) {
        }
        return null;
    }
...
//
    ...
    try (Connection connection = getConnection();
        ...
        try (ResultSet rs = ps.executeQuery()) {
            ...
            // Gửi feedback cho tất cả client đã kết nối
            for (ClientService client : ClientService.lstSocket) {
                client.sendMessage(this.feedBack);
            }
        }
    } catch (SQLException ex) {
        Logger.getLogger(ServerFrm.class.getName()).log(Level.SEVERE, null, ex);
