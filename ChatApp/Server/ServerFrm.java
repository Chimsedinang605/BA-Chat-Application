...
//    }
    private boolean checkLogin(String userName, String passWord) {
        for (User user : _lstUsers) {
            if (user.getUserName().equals(userName) && user.getPassWord().equals(encryptPassword(String.valueOf(passWord)))) {
                return true;
            }
        }
        return false;
    ...
    private void changePassword(String request) {
        ...
        if (checkUserName(request)) {
            try {
                String encryptedPassword = encryptPassword(newPass);
...
//
    ...
    private void sendDialogue(String request) {
        ...
        try (Connection connection = getConnection(); PreparedStatement ps = connection.prepareStatement("CALL GETDIALOGUE(?, ?)")) {
